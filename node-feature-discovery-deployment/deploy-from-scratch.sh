...
log "Ensuring namespace exists and PodSecurity labels are set..."
...
if kubectl get daemonset "$WORKER_DAEMONSET" -n "$NAMESPACE" >/dev/null 2>&1; then
  # If worker was previously blocked by PodSecurity, explicit restart clears controller backoff.
